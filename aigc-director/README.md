...
将本项目作为 Skill 注册到 OpenClaw 平台，OpenClaw 会自动安装依赖并启动服务。

#### 1. 注册 Skill

在 OpenClaw 平台注册 `aigc-director/` 目录作为 Agent Skill。

#### 2. 配置 API Key

在 `aigc-claw/backend/.env` 中配置所需的 API Key（参考下方配置说明）。

#### 3. 启动服务

OpenClaw 会自动启动后端服务，用户可直接通过对话发起视频生成任务。
